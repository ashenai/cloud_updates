...
class Theme(db.Model):
    """Theme model for storing themes generated by LLM."""
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text, nullable=False)
    provider = db.Column(db.String(50), nullable=False)  # aws or azure
    services = db.Column(db.JSON, nullable=False)  # List of services involved
    update_count = db.Column(db.Integer, nullable=False)
    score = db.Column(db.Float, nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    def __repr__(self):
        return f'<Theme {self.name} ({self.provider})>'
